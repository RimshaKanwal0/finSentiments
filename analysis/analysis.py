...
#         for word in tokens
#         if len(word) > 1 and word.lower() not in stopwords.words('english') and word.lower() not in custom_stopwords
#     ]
#     tokens = [word for word in tokens if len(word) > 1]
#     tokens = convert_numbers_to_text(tokens)
#     return ' '.join(tokens)
...
#
# # Train a SentencePiece model for Tokenization
# spm.SentencePieceTrainer.train(input='E:/django/sentimentproject/finSentiments/dataset/data.csv', model_prefix='mymodel',
...
#     tokens = [word for word in tokens if len(word) > 1]
#     tokens = convert_numbers_to_text(tokens)  # Convert numbers to text
#     preprocessed_text = ' '.join(tokens)
#     return preprocessed_text
#
#
...
#     df['Preprocessed_Sentence'] = df['Sentence'].apply(preprocess_text)
#     return df['Preprocessed_Sentence']
#
#
# # Function to prepare data for training
# def prepare_data(df, preprocessing_flag=1):
#     if preprocessing_flag == 1:
#         X = df['Preprocessed_Sentence']
#     else:
#         X = df['Sentence']
...
#                                                         random_state=42)
#
#     # Convert to dense numpy array for model compatibility
#     X_train_tfidf_dense = X_train.toarray()
#     X_test_tfidf_dense = X_test.toarray()
#
#     return X_train_tfidf_dense, X_test_tfidf_dense, y_train, y_test
#
#
# # Common Function for Metrics Calculation
...
#     }
#
# # Function for Training and Evaluating Individual Classifier
# def train_and_evaluate_individual_classifier(X_train, y_train, X_test, y_test, classifier_name, additional_info):
#     classifiers = {
#         'svm': SVC(),
#         'sgd': SGDClassifier(),
#     }
...
#     metrics.update({'Classifier': classifier_name, 'Additional Info': additional_info})
#
#     return metrics
#
# # Function for Training and Evaluating Blended Classifier
# def train_and_evaluate_blended_classifier(X_train, y_train, X_test, y_test, classifier_combo, additional_info):
#     start_time = time.time()
#
#     classifiers = {
#         'svm': SVC(),
...
#     if any(name not in classifiers for name in classifier_combo):
#         raise ValueError(f"Unsupported classifier in combination: {classifier_combo}")
#
